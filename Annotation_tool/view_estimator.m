...
function [azimuth, elevation, azi_co, ele_co, distance, focal, px, py, theta,...
    error, interval_azimuth, interval_elevation, num_anchor, ob_index] = ...
    view_estimator(cls, record, cad_bunch)

ttnum = numel(record.objects);
j = 1;
for i = 1:ttnum
    if(strcmp(record.objects(1,i).class,cls))
        ob_index(j) = i;
        j = j + 1;
    end
end
...
for i = 1:num_ob
    ...
    x3d = [];
    for j = 1:part_num
        if anchors.(pnames{j}).status == 1  % filters the point not clicked on the image
    ...
    if num_anch < 2
        ...
        v0(2) = e*pi/180;
        margin = 22.5;
        eextent = [max(e-margin,0)*pi/180 min(e+margin,90)*pi/180];        
        % distance
        dextent = [0, 100];
...
rescale = 50;
lb = [center(1)-width/rescale center(2)-height/rescale];
ub = [center(1)+width/rescale center(2)+height/rescale];
